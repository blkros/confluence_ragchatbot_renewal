...
async def _llm_direct_answer(limited_msgs: list[dict], req: "ChatReq") -> str:
    ...
    sysmsg = {
        ...
        "content": (
            ...
            "질문과 관련 없는 내용은 절대 포함하지 말고, 오직 질문에 대한 답변만 하세요."
        )
    }
...
def build_system_with_context(ctx_text: str, mode: str, force_summary: bool = False) -> str:
    ...
    return (
        ...
        "- 약어나 전문 용어(예: OCPP, EMS, API 등)가 질문에 포함되면, 해당 용어의 일반적 정의를 한 줄로 먼저 설명한 뒤 컨텍스트의 구체적 내용을 이어서 답변한다.\n"
        "- 컨텍스트가 XML, 소스 코드, raw 데이터를 포함할 경우 그대로 복사하지 말고 핵심 구조와 의미를 사람이 읽기 쉽게 요약한다.\n"
        "- 컨텍스트가 질문과 명백히 무관하거나, 질문 주제를 직접 설명하지 않고 단순 언급만 하는 경우(예: 업무일지·회의록에서 항목명으로만 적힌 경우) 무시하고 일반 지식으로 답변한다.\n"
